...
**Possible Answer:** <Spoiler>Any continuous mapping from the Möbius strip into the half 3D space where $z \ge 0$, which maps the boundary of the Möbius strip to the $xy$-plane, must have two inputs which map to the same output. (More fancifully, such a mapping cannot be injective.)</Spoiler>
